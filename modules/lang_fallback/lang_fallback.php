<?php
/** Switch to another language should the desired page not be available in the
  * selected language.
  *
...
class Lang_Fallback extends Module {
    ...
    function lang_fallback_step($params) {
        ...
        if ($content && $content->active) {
            ...
            Log::debug("Page unavailable in language $lg, lang_fallback module trying to find content in another language");
            ...
            foreach(explode(",", get($params['server'], 'HTTP_ACCEPT_LANGUAGE')) as $user_language) {
                ...
                foreach($all_content as $cancontent) {
                    if ($cancontent->active && $user_lg == $cancontent->lg) {
