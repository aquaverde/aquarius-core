...
class Formtype_global_legend_file extends Formtype_File {
    ...
    function db_set($values, $formfield) {
        ...
        if (strlen($legend) == 0) {
            $DB->query("DELETE FROM file_legend WHERE file='$escaped_path'");
        } else {
            $escaped_legend = mysql_real_escape_string($legend);
            $DB->query("REPLACE file_legend SET file='$escaped_path', legend='$escaped_legend'");
        }
    ...
    function db_get($values, $formfield) {
        ...
        $values = parent::db_get($values, $formfield);

        $escaped_path = mysql_real_escape_string(get($values, 'file'));
        $global_legend = $DB->singlequery("SELECT legend FROM file_legend WHERE file='$escaped_path'");
